...
'''Write a script that inputs a birthdate and time, then calulates and displays:
- The day of the week the person waws born.
- The number of years, months, and days the person has been alive.
- Total months, days, hours, minutes, and seconds the person has been alive.
- Use the %x and %X format specifiers.
* Results printed using locale-specific formatting -- considering the user language, country and time zone.'''
def date_time():
    input_prompts = ['month: ', 'date: ', 'year: ', 'hour: ', 'minute: ', 'AM/PM: ']
    for x in input_prompts:
        filled_prompts = []
        filled_prompts.append(input(x).strip())
        return filled_prompts
    now = dt.datetime.now()
    birth_dt = dt.datetime.strptime(filled_prompts)
    age_delta = now - birth_dt
    age_delta1 = Enum('delta_enum', ['%A', '%b %d', '%Y', '%I:%M %p'])
    for x in age_delta1:
        print(f'{age_delta.strftime(x)}.\n')
# -------------------------------------------------------------------------------

# receives at least one argument. Can pass 10, 20, and 30 collectively or individually.
# Can pass sequence of integers produced by range(1, 6, 2)
def product(arg_required, *args):
    result = arg_required
    for arg in args:
        try:
            for x in arg:
